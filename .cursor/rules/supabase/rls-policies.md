...
When generating RLS policies, ensure:

- ✅ Use `CREATE POLICY` or `ALTER POLICY` only
- ✅ Use `auth.uid()` instead of `current_user`
- ✅ SELECT policies have USING only
- ✅ INSERT policies have WITH CHECK only
- ✅ UPDATE policies have both USING and WITH CHECK
- ✅ DELETE policies have USING only
